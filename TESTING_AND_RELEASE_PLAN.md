...
#### 12.多显示器选择
**描述**：（两个显示器下）使用多显示器播放的情况无非就两种，两个显示器播放的相同或者不同；
- 相同情况下使用这样的指令：
  ```shell
  ...
  ```
- 不同情况下只能~一个壁纸/一个屏幕对应一个进程了
~~可参考7的指令使用~~

那么在双显示器下，就应该有一个`same`/`diff`的切换方式，让用户选择两张屏幕的壁纸是否一样，如果是`same`就直接一键同步两屏幕的壁纸，简化使用；若是`diff`就让用户自行选择两张壁纸（这个切换按键可以与屏幕切换器挨着，放在右边~~，当然，单显示器下这个按键没有存在的必要,可以隐藏~~）

推广到更多显示器下，可能会有更加复杂的情况，比如三显示器下可能会有`2+1`的情况，这时不若给`Apply Wallpaper`键做一个下拉抽屉：`Apply Wallpaper (on):`,`on`是抽屉的开关，打开抽屉是对应的具体各个显示器的名称，用户通过选中需要应用的显示器随后再apply，让壁纸一次性应用在多个显示器上，更加方便操作(这样的话是三个屏幕使用两个进程)

当然，在识别到用户在`diff`模式下仍旧使用了相同的壁纸，则自动切换到`same`模式

~~感觉并不太实用~~

#### 13.预览图
**描述**：部分壁纸的gif第一帧或前几帧是黑屏的，这个时候显示在前端预览也是纯黑的，用户看不到东西。
**建议**是要么直接换成gif播放，要么改为显示gif的第1+n帧/中间帧/最后一帧

#### 14.总数显示
- 在合适位置(暂时设想与顶栏的CURRENTLT USING一排，但是在这一排的最右侧)显示壁纸总数与当前使用的壁纸的顺序位次之比（N/M），用稍微醒目一点的字，比如说可以用跟下一排显示的壁纸名字一个颜色
