...
- ✅ **Web (.html)**: 支持良好，绝大多数 Web 壁纸可正常运行。
- ⚠️ **Scene (.pkg)**: **支持有限**。
  - 简单的 2D/3D 场景通常正常。
...
- **⚠️ OpenGL/GLX 兼容性**：部分旧式特效依赖 GLX (X11)，在纯 Wayland 环境下可能因 `Failed to initialize GLEW: No GLX display` 错误导致渲染层级异常（例如：**音频响应波形**虽然已计算但被背景遮挡或完全透明）。
