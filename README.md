...
- ❌ **实时应用**：修改属性后点击 Apply 按钮，壁纸参数未生效（属性值已保存到JSON但对正在播放的壁纸无影响）
...
1. **壁纸属性编辑无效**
   - 现象：修改侧边栏属性控件后，属性值虽然保存到 `config.json`，但对正在播放的壁纸无影响
   - 影响：所有壁纸属性编辑功能（Slider/Toggle/Color/Combo）
   - 状态：配置文件正确，但壁纸引擎未正确应用新参数

2. **CLI `--toggle` 命令功能不完整**
   - 现象：`--toggle` 命令只能隐藏窗口，无法显示已隐藏的窗口
   - 解决方案：使用 `--show` 显式显示窗口
   - 示例：`python3 wallpaper_gui.py --show`

3. **`--apply-last` 行为说明**
   - 行为：`--apply-last` 应用的是**上次关闭应用时保存的壁纸**（`config.json` 中的 `lastWallpaper`）
   - 注意：如果应用运行中点击 GUI 界面的 Stop 按钮停止壁纸，然后执行 `--apply-last`，不会恢复刚刚 stop 的壁纸，而是恢复上次关闭应用时保存的壁纸
   - 用途：适用于重启系统后恢复上次的壁纸，不适用于临时暂停后恢复
...
| 壁纸浏览 | ✅ 已验证 | 基本功能正常 |
| 侧边栏详情 | ✅ 已验证 | 显示正常 |
| 应用壁纸 | ✅ 已验证 | 点击 Apply 正常应用 |
| 随机选择 | ✅ 已验证 | I'm feeling lucky 正常 |
| 快速刷新 | ✅ 已验证 | Refresh 按钮正常 |
| 壁纸属性编辑 | ⚠️ 部分失效 | 值保存但壁纸未生效 |
| 多显示器支持 | ⚠️ 未测试 | 单显示器环境无法验证 |
| 搜索功能 | ✅ 已验证 | 实时过滤正常 |
| 配置管理 | ✅ 已验证 | 各设置项保存正常 |
| 后台启动 `--hidden` | ✅ 已验证 | 后台运行正常 |
| CLI `--show` | ✅ 已验证 | 显示窗口正常 |
| CLI `--hide` | ✅ 已验证 | 隐藏窗口正常 |
| CLI `--toggle` | ❌ 部分失效 | 只能隐藏，不能显示 |
| CLI `--refresh` | ⚠️ 未测试 | 未实际验证 |
| CLI `--apply-last` | ✅ 已验证 | 功能正常（按设计行为） |
| CLI `--quit` | ✅ 已验证 | 退出应用正常 |
...
**最后更新**：2026-01-22
**版本**：0.3-beta（壁纸属性编辑、多显示器支持、搜索功能部分实现，存在已知问题）
