...
### 架构重构与性能极客优化
- 纯 Rust 原生系统托盘:
  - 使用纯 Rust 和 ksni (纯 DBus 协议) 库彻底重写了系统托盘组件，全面替代原有的 Python 子进程方案。
  - 极致内存优化：得益于彻底剥离 Python 解释器和庞大的 GTK3/GI 动态绑定库，托盘进程的物理内存占用暴降约 95%（从约 70MB 缩减至惊人的 3.8MB 左右）。
  - 彻底告别沙盒与依赖痛点：托盘现已作为独立的极简二进制文件运行。从根本上解决了 AppImage 环境下的 FUSE 挂载点断链问题，并完美规避了同一进程下 GTK3 与 GTK4 的命名空间冲突崩溃。
  - 精准进程嗅探：重构了底层的引擎运行状态检测逻辑。通过精确解析 /proc 中的实际可执行文件路径，完美修复了托盘右键菜单中 "Play/Stop" 状态卡死（误把托盘自身识别为引擎）的 Bug。

### 交互体验与窗口管理
- 完美的窗口焦点唤醒 (Focus & Raise):
  - 修复了在多工作区（Workspace）或大量窗口遮挡状态下，点击托盘 "Show Window" 无法将已打开的界面强制拉至前台的痛点。
  - 引入了带有 100ms 延迟的 window.present() 强行唤醒机制。该机制成功绕过了 Wayland 极其严格的“防焦点窃取 (Focus Stealing Prevention)”安全限制，确保窗口管理系统能可靠地将 GUI 瞬间跨桌面置顶并夺取焦点。
