...
## v1.0.0-pre
### Architecture & Extreme Performance
- Native Rust System Tray:
  - Completely rewrote the system tray component from a Python subprocess to a pure Rust implementation using the ksni (DBus) library.
  - 95% Memory Reduction: By eliminating the overhead of the Python interpreter and GTK3/GI dynamic bindings, the tray's memory footprint has been slashed from ~70MB down to an astonishing ~3.8MB.
  - Sandbox & Dependency Freedom: The tray is now compiled as a standalone, statically linked binary. This completely resolves previous AppImage FUSE mount breaking issues and strictly bypasses the fatal GTK3/GTK4 namespace conflicts.
  - Smarter Process Detection: Refined the backend engine detection logic by accurately parsing executable arguments from /proc. This fixes the bug where the "Play/Stop" toggle menu would incorrectly get stuck in the "Stop" state.

### UX & Window Management
- eliable Window Focus & Wake-up:
  - Fixed a critical UX issue where clicking "Show Window" from the tray would fail to bring the application to the foreground if the window was already open in the background or on another virtual workspace.
  - Implemented a delayed window.present() invocation (with a 100ms offset). This reliably bypasses Wayland's strict "Focus Stealing Prevention" security mechanism, forcing the window manager to smoothly raise and focus the GUI across multiple workspaces.
