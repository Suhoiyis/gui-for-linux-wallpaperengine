...
- **壁纸昵称全局支持 (Full Nickname Support)**:
  ...
  - 交互升级：在右侧详情栏新增 ✏️ 编辑按钮，并在右键菜单中增加“设置别名”选项。
...
- **布局一致性**: 将侧边栏编辑按钮推至最右侧，并提升了原名小字的透明度（0.5 -> 0.7）以增强可见性。
- **管理弹窗优化**: 增强了父窗口获取的健壮性，显式传递主窗口引用，解决了“Manage Nicknames”按钮在某些情况下无反应的问题。
- **动态预览兼容性**: 优化了弹窗中 GIF 缩略图的加载逻辑，通过 `GdkPixbufAnimation` 确保动态壁纸预览不再显示为空白。
