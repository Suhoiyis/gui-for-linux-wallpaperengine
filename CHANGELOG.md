...
### 全局导航与关于对话框优化 (Global Navigation & About Dialog)
- **汉堡菜单精简 (Hamburger Menu Refinement)**:
  - **功能收纳**: 将“重启应用”按钮从顶栏移至汉堡菜单内，移除图标改为纯文字显示，释放顶栏视觉空间。
  - **去冗余**: 移除了壁纸列表工具栏上的“刷新”按钮，统一通过菜单中的“Refresh Library”执行。
  - **样式优化**: 修正了菜单按钮的“双层边框”瑕疵；为 **Restart** 和 **Quit** 增加了粗体强调，其中 **Quit** 设为红色。
  - **交互一致性**: 统一了顶栏右侧按钮组的视觉间距。
- **About 对话框现代化**:
  - **组件升级**: 从已弃用的 `Adw.AboutWindow` 迁移至最新的 `Adw.AboutDialog` (Libadwaita 1.5+)。
  - **动态日志同步**: 实现了 About 窗口自动读取并解析 `CHANGELOG.md` 最新条目的功能，且支持中文内容的实时完美排版。
  - **专业调试信息**: 新增了标准化的系统环境报告，通过 Unicode 符号优化了纯文本显示效果，方便一键查看/复制。
  - **视觉提升**: 关于窗口现在显式展示 iOS 风格的圆角应用图标 (`GUI_rounded.png`)。
- **稳定性修复**: 彻底解决了 About 窗口中由于 `<b>` 标签及未转义字符（如 `&`）导致的 XML 解析崩溃问题。
