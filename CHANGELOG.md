...
- **布局优化 (UI Layout Optimization)**:
  - **紧凑模式侧边工具栏**: 将“停止”、“随机”和“跳转当前”按钮移动至预览图右侧的垂直工具栏，利用 $X/Y$ 下方的空间，使下方信息展示更完整。
  - **停止按钮视觉强化**: 优化 CSS 规则，确保所有模式下的“停止”按钮图标显示为红色。
  - **极致紧凑导航设计**: 进一步优化了小窗模式的序号跳转框，采用无边框输入框 ($X$) 紧贴静态标签 ($/Y$) 的设计，实现视觉统一。
- **功能增强 (Feature Enhancement)**:
  - **普通模式全局跳转**: 将数字序号跳转功能引入普通模式的“正在使用”状态栏和侧边栏，支持全局快速定位。
- **交互式导航 (全局)**：
  - 在精简模式和普通模式（状态面板和侧边栏）中添加了数字索引跳转输入框 ($X/Y$)。用户可以输入数字并按回车键跳转到特定壁纸。
  - 实现了自动限制（如果输入超过总数则跳转到最后一项）以及所有导航方式之间的自动同步。
  - 输入框采用超紧凑、无边框设计，以保持界面整洁。
- **UI 一致性 (精简模式)**：
  - 将自定义的 MenuButton 屏幕选择器替换为原生的 `Gtk.DropDown`，并居中显示在标题栏中，以获得更好的视觉平衡和与普通模式的一致性。
