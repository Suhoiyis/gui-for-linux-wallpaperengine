...
- **紧凑模式改进 (Compact Mode Improvements)**：
  ...
  - **UI 一致性 (UI Consistency)**：将紧凑模式下的自定义 `MenuButton` 屏幕选择器替换为原生 `Gtk.DropDown`，与主窗口的交互风格保持一致。选择器现在在标题栏中视觉居中，使布局更平衡。
- **新增序号跳转功能 (Index Jump Feature)**：
  - **交互式导航**：在紧凑模式（Compact Mode）右上角新增了独立的序号跳转输入框。用户可以输入特定数字并按回车键直接跳转到对应壁纸。
  - **UX 优化**：
    - **自动钳位逻辑**：实现自动钳位（Auto-clamping），若输入序号超过总数则自动跳转至最后一张。
    - **极简设计**：采用无边框输入框显示当前序号 ($X$)，后接静态标签显示总数 ($/Y$)，确保界面整洁且视觉统一。
    - **自动同步**：通过其他方式导航时，输入框内容会自动同步更新。
