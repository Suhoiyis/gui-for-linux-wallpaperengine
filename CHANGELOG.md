...
### 改进 (Compact Mode)
- **Wayland/Niri 兼容性**：修复了在 Wayland (Niri) 环境下重启应用后窗口不可见的问题，通过延迟调用 `present()` 确保窗口正确显示
- **循环导航**：实现了缩略图列表和快捷键（左右方向键）的循环滚动，首尾相接，操作更顺滑
- **尺寸优化**：将 Compact 窗口默认尺寸从 300x800 减小至 300x600，缩略图尺寸从 50px 减小至 40px，动作按钮从 34px 减小至 30px，整体视觉更紧凑
- **视觉增强**：恢复了壁纸 ID 的显示，并采用精致的蓝色胶囊 (Capsule) 样式，支持点击复制
- **交互优化**：在缩略图两侧增加了显式的“上一个/下一个”导航按钮，提升触控和鼠标操作的直观性
