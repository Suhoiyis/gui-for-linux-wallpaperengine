...
### 全局导航微调 (Global Navigation Refinement)
- **汉堡菜单视觉优化**:
  - 移除了菜单项按钮的“双层边框”视觉瑕疵，通过 CSS 穿透强制将内部按钮背景透明化。
  - 优化了菜单项的内边距 (Padding) 和间距 (Margin)，使列表更加紧凑精致。
  - 增强了字体样式区分：**Restart** 和 **Quit** 选项强制使用 **粗体** 显示，以突显重要操作。
- **顶栏布局优化**: 为紧凑模式切换按钮增加了右侧间距，使其与新加入的汉堡菜单保持视觉平衡。
