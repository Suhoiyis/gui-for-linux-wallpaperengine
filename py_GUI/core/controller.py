...
from py_GUI.core.state import StateManager, AppStateBus
...
class WallpaperController:
    def __init__(
        ...
        signal_bus: AppStateBus | None = None,
    ):
        self.config = config
        ...
        self.state = state_manager or StateManager(signal_bus)
        ...
        self.perf_monitor = PerformanceMonitor(signal_bus=signal_bus)

        if shutil.which("xvfb-run"):
            self.log_manager.add_info(
                "Xvfb detected: Silent screenshots enabled", "Controller"
            )
        else:
            self.log_manager.add_info(
                "Xvfb not found: Screenshots will spawn a window", "Controller"
            )
    ...
    def apply(
        ...
        if screens:
            ...
            last = self.state.get_last_screen()
            if not last or last == "None":
                last = (
                    self.screen_manager.get_primary_screen()
                    or self.screen_manager.get_first_screen()
                    or "eDP-1"
                )
            target_screens = [last]

        active_monitors = self.state.get_active_monitors()

        for s in target_screens:
            active_monitors[s] = wp_id

        self.state.set_active_monitors(active_monitors)
        self.state.set_last_wallpaper(wp_id)

        if len(target_screens) == 1:
            self.state.set_last_screen(target_screens[0])

        self.log_manager.add_info(
            f"Applying wallpaper {wp_id} to {target_screens}", "Controller"
        )
        ...
        if self.history_manager:
            try:
                ...
                self.log_manager.add_error(
                    f"Failed to record wallpaper in history: {e}", "Controller"
                )
    ...
    def restart_wallpapers(self):
        ...
        valid_monitors = {
            scr: wid for scr, wid in active_monitors.items() if scr in connected_screens
        }

        # If monitors were removed, update config
        if len(valid_monitors) != len(active_monitors):
            self.log_manager.add_info(
                f"Removing disconnected screens from config. Valid: {list(valid_monitors.keys())}",
                "Controller",
            )
            self.state.set_active_monitors(valid_monitors)
            active_monitors = valid_monitors

        if not active_monitors:
            self.log_manager.add_info(
                "No active wallpapers for connected screens.", "Controller"
            )
            return
        ...
        self.log_manager.add_debug(
            f"Building command. Silence mode: {is_silent_mode} (Raw: {silence_cfg})",
            "Controller",
        )
        ...
        for wid in set(active_monitors.values()):
            ...
            for prop_name, prop_value in user_props.items():
                ...
                formatted_value = self.prop_manager.format_property_value(
        ...
        try:
            ...
            self.current_proc = subprocess.Popen(
                cmd, stdout=self.engine_log, stderr=self.engine_log
            )
    ...
    def take_screenshot(
        self, wp_id: str, output_path: str, delay: Optional[int] = None
    ):
    ...
    def stop(self):
        ...
        if self.current_proc:
            ...
            if (
                hasattr(self, "engine_log")
                and self.engine_log
                and not self.engine_log.closed
            ):
                try:
                    self.engine_log.close()
                except Exception:
