...
class CompactWindow(Gtk.ApplicationWindow):
    ...
    def _build_ui(self):
        ...
        self.screen_dd = Gtk.DropDown.new_from_strings(
            self.screen_manager.get_screens()
        )
    ...
    def _on_apply_clicked(self, btn):
        if self.selected_wp:
            ...
            self.controller.state.set_last_screen(self.target_screen)
    ...
    def _on_id_clicked(self):
        if self.selected_wp:
            ...
            self.lbl_id.set_tooltip_markup(
                "<span foreground='#22c55e' weight='bold'>Copied!</span>"
            )
            GLib.timeout_add(
                1500, lambda: self.lbl_id.set_tooltip_text("Click to copy ID") or False
            )
