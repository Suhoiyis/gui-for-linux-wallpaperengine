...
class Sidebar(Gtk.Box):
    ...
    def build_ui(self):
        ...
        self.btn_compact_stop.connect(
            "clicked",
            lambda b: self._on_stop_cb() if callable(self._on_stop_cb) else None,
        )
        self.btn_compact_lucky.connect(
            "clicked",
            lambda b: self._on_lucky_cb() if callable(self._on_lucky_cb) else None,
        )
        self.btn_compact_jump.connect(
            "clicked",
            lambda b: self._on_jump_cb() if callable(self._on_jump_cb) else None,
        )
    ...
    def update(self, wp_id: Optional[str], index: int = 0, total: int = 0):
        ...
        if self.nickname_manager:
            display_title, returned_original = self.nickname_manager.get_display_name(
                wp
            )
        ...
        if self.selected_wp:
            ...
            if mode == "same" and self.available_screens:
                ...
                self.log_manager.add_info(
                    f"Applied wallpaper {self.selected_wp} to ALL screens: {self.available_screens}",
                    "Sidebar",
                )
            else:
    ...
    def on_folder_clicked(self, gesture, n_press, x, y):
        if self.selected_wp:
            ...
            self.log_manager.add_info(
                f"Copied ID to clipboard: {self.selected_wp}", "GUI"
            )

            # Temporary green tooltip feedback
            self.lbl_folder.set_tooltip_markup(
                "<span foreground='#22c55e' weight='bold'>Copied!</span>"
            )
