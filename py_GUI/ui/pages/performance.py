...
class PerformancePage(Gtk.Box):
    ...
    def build_ui(self):
        ...
        desc = Gtk.Label(
            label="Real-time resource usage of Wallpaper Engine components."
        )
        ...
        self.total_cpu_chart = Sparkline(
            color=(0.937, 0.267, 0.267), height=150, title="Total CPU History"
        )
        ...
        self.total_mem_chart = Sparkline(
            color=(0.16, 0.5, 0.73), height=150, title="Total Memory History"
        )
    ...
    def _update_ui(self, stats: Dict):
        ...
        for category, data in details.items():
            ...
            self._update_process_row(
                self.process_widgets[pid],
                category,
                data,
    ...
    def _create_process_row(self, category: str, data: Dict) -> Gtk.Box:
        ...
        main_box.cpu_chart = Sparkline(
            color=(0.937, 0.267, 0.267), height=120, title="CPU Usage"
        )
        ...
        main_box.mem_chart = Sparkline(
            color=(0.16, 0.5, 0.73), height=120, title="Memory Usage"
        )
                    ...
                    if active_monitors:
                        ...
                        for screen, wp_id in active_monitors.items():
                            ...
                            if hasattr(self.controller, "wp_manager"):
                                ...
                                if wp:
                                    if hasattr(self.controller, "nickname_manager"):
                                        display_name, secondary_text = (
                                            self.controller.nickname_manager.get_display_name(
                                                wp
                                            )
                                        )
                                    else:
                            ...
                            if preview_path and hasattr(self.controller, "wp_manager"):
                                texture = self.controller.wp_manager.get_texture(
                                    preview_path, size=64
                                )
                                if texture:
    ...
    def build_screenshot_history_panel(self):
        ...
        desc = Gtk.Label(
            label="Resource usage from recent screenshot captures (last 10)."
        )
        ...
        self.screenshot_history_box = Gtk.Box(
    ...
    def _refresh_screenshot_history(self):
        while self.screenshot_history_box.get_first_child():
            self.screenshot_history_box.remove(
                self.screenshot_history_box.get_first_child()
            )
    ...
    def refresh_screenshot_history_safe(self):
        if hasattr(self, "screenshot_history_box"):
            self._refresh_screenshot_history()
    ...
    def _create_screenshot_history_row(self, record: Dict) -> Gtk.Box:
        ...
        if hasattr(self.controller, "wp_manager"):
            ...
            if wp:
                ...
                if hasattr(self.controller, "nickname_manager"):
                    display_name, secondary_text = (
                        self.controller.nickname_manager.get_display_name(wp)
                    )
                else:
        ...
        open_folder_btn.connect(
            "clicked", lambda _: self._open_screenshot_folder(output_path)
        )
        ...
        open_image_btn.connect(
            "clicked", lambda _: self._open_screenshot_image(output_path)
        )
