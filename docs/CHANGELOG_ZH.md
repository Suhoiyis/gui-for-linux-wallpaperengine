...
## v1.0.0-pre (Latest Update)
### 核心功能突破：双向 IPC 与多维状态感知
- 工业级双向 IPC 通信架构:
  - 建立了基于 Unix Domain Sockets (UDS) 的全双工通信链路。在保留 Rust 托盘极简体积的同时，赋予了其与 Python 主进程实时对话的能力。
  - 引入了状态同步心跳机制与防抖 (Diffing) 逻辑，确保托盘 Tooltip 始终与 GUI 界面保持毫秒级同步，且在状态未改变时实现 0 Socket 负载。

- 动态多维 Tooltip 提示系统:
  - 多显示器全兼容: 托盘现能智能感知多屏布局。在多屏环境下，通过竖向列表优雅展示每个显示器的独立运行状态，彻底告别单屏显示局限。
  - 别名系统深度集成: 托盘状态完美支持自定义别名 (Nickname)。通过直接抓取 GUI 渲染层文本，确保托盘显示的壁纸名称与用户自定义的外号保持 100% 视觉一致。
  - Pango Markup 视觉美化: 利用 Pango 标记语言对 Tooltip 进行了排版优化。通过加粗标题、斜体状态名以及~~自定义行间距“垫片”~~，在系统底层气泡框中实现了极具层次感的视觉体验。

### 稳定性与健壮性提升
- 智能 Socket 轮询重试: 针对冷启动场景开发了 Socket 自动重试机制。有效解决了因 Rust 托盘初始化时滞导致的 Python 首次状态推送丢失问题。
- 安全沙盒强化: 为所有 UDS 管道强制实施了 `0o600` 权限控制，确保 IPC 通信仅限当前用户，从底层防御潜在的本地提权控制风险。
- 进程生命周期优化: 修复了 `do_activate` 阶段的重入 Bug。通过严格的初始化状态校验，确保在重复激活应用时不会导致托盘组件与 IPC 服务的冗余创建。

### 交互体验与窗口管理
- **托盘原生左键交互与智能切换 (Smart Toggle)**:
  - 深度利用 Rust `ksni` 库的 `activate` 接口，为系统托盘新增了原生级别的左键单击响应。彻底告别繁琐的右键菜单，实现一键极速呼出/隐藏主界面。
  - 引入了符合现代桌面系统直觉的“智能切换 (Smart Toggle)”逻辑：当主界面被其他窗口遮挡、或位于其他虚拟工作区时，单击托盘会将其瞬间跨屏幕强行置顶并夺取焦点；仅当窗口已经处于最顶层激活状态 (`is_active()`) 时，才会执行平滑隐藏。极大提升了高强度多任务处理时的操作连贯性。

### 视觉体验与底层资源优化
- **状态感知动态托盘图标 (State-Aware Tray Icon)**:
  - 彻底打通了从主引擎到托盘视觉的反馈链路。托盘图标现能根据壁纸的实时播放状态，智能且无缝地在彩色（运行中）与黑白灰阶（已停止）之间切换。用户无需悬停鼠标或打开主界面，仅凭视觉余光即可精准掌握引擎的耗电/运行状态。
- **FUSE 沙盒穿透与图标直通渲染机制**:
  - 针对 AppImage 环境下由于 FUSE 挂载点权限隔离，导致部分桌面环境（DE）无法读取托盘图标的系统级痛点，开发了“启动时安全区释放”黑魔法。应用启动时会自动将托盘资源“偷渡”至用户目录下的 `~/.local` 标准路径。
  - 通过向 Rust 底层直接传递安全区的绝对路径，不仅完美绕过了 AppImage 沙盒的底层拦截，还彻底免除了对桌面环境 `icon-theme.cache` 刷新机制的依赖，实现了任意 Linux 发行版下的 100% 图标渲染成功率。
- **像素级资源降维优化 (Pixel-Perfect Optimization)**:
  - 废弃了以往将 2000px 级主程序高分大图强行塞给托盘、再交由系统动态缩放的粗暴做法。
  - 现已全面换用专门绘制的 59x64 极致精简托盘小图。这一架构级改动将 Rust 托盘守护进程的显存/内存开销从数十兆级断崖式压缩至 KB 级别，同时大幅提升了在 4K 高分屏或低分屏下的抗锯齿边缘锐利度，告别“糊成一团”的缩放失真。
