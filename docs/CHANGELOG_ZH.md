...
从 Python/GTK 完全重写为 Tauri/React/Rust：

- **前端**: Python 3.10+ / GTK4 / Libadwaita → React 19 / TypeScript / Tailwind CSS / shadcn/ui
- **后端**: Python (子进程管理) → Rust (`lwg-core` crate)
- **IPC**: 抽象套接字 (托盘 ↔ GUI) → Tauri 命令与事件
- **性能**: 解释器开销 → 原生 Rust 性能

### 新功能

- **播放列表系统**: 创建、编辑和循环播放自定义壁纸集合，带有专用侧边栏
- **收藏系统**: 一键收藏，快速访问最常用的壁纸
- **现代化 UI**: 使用 React 和 Tailwind CSS 构建的更快、更响应的界面
- **增强的性能监控**: 实时 CPU/内存图表，支持进程级别分解（前端、后端、引擎）
- **Tauri v2**: 利用最新的 Tauri 特性，提供更好的系统集成和安全性
- **XDG 合规**: 配置、状态和缓存文件现在遵循 XDG 基础目录规范

### 配置变更

配置从单个 `config.json` 拆分为六个专用文件：

| 文件 | 位置 | 用途 |
|------|------|------|
| `config.json` | `~/.config/linux-wallpaperengine-gui/` | 主设置 |
| `state.json` | `~/.local/state/linux-wallpaperengine-gui/` | 运行时状态 |
| `nicknames.json` | `~/.local/share/linux-wallpaperengine-gui/` | 自定义名称 |
...
设置标签页从 General/Audio/Advanced 更改为：

| 标签页 | 设置内容 |
|--------|----------|
| **播放** | FPS、缩放、钳制、视差、粒子、轮换、Wayland 微调 |
| **显示** | 音量、静音 |
| **系统** | 路径、自启动、截图、别名、收藏 |
| **日志** | 实时日志查看器，支持过滤 |
...
- 仅支持 `--hidden` 参数用于后台启动
- 移除了旧版 CLI 命令（`--show`、`--hide`、`--toggle`、`--random`、`--stop`、`--quit`）

### 移除的功能

- 系统强调色同步（主题现在仅使用系统深色/浅色偏好）
- Web 壁纸属性注入面板（Wayland 限制）
- 桌面快捷方式自动创建（请使用 AppImage 或包管理器）

### 依赖

- 需要 `webkit2gtk-4.1`（不再需要 `libadwaita` 或 `gtk4`）
...
<details>
<summary>展开查看旧版更新日志</summary>

## v1.0.0-pre (2026-02-24)
### 托盘交互与视觉体验 (Tray UX & Visuals)
* **左键智能切换 (Smart Toggle)**: 托盘新增原生级左键单击响应。单击图标即可极速唤醒或隐藏主界面；当窗口被遮挡或在其他工作区时，会自动跨屏强行置顶夺取焦点，高强度多任务切换更流畅。
* **状态感知动态图标 (State-Aware Icon)**: 托盘图标现能根据壁纸运行状态，智能且无缝地在彩色（运行中）与黑白灰阶（已停止）之间切换。无需悬停或打开界面，用余光即可精准掌握引擎状态。
* **多屏多维 Tooltip 提示**:
* 完美兼容多显示器布局，悬浮提示框通过竖向列表优雅展示每个屏幕的独立运行状态。
* 深度集成别名系统，并利用 Pango Markup 优化排版结构（加粗标题、斜体状态名），视觉层次更分明。

### 底层架构与性能优化 (Architecture & Performance)
* **全双工双向 IPC 通信**: 基于 Unix Domain Sockets (UDS) 建立主进程与托盘的极速通信链路。配合状态防抖同步，实现毫秒级 UI 响应，且在状态闲置时保持 0 Socket 负载。
* **像素级内存降维优化**: 彻底废弃以往将主程序 2000px 大图交由系统强行缩放的粗暴做法，换用专制的 59x64 极致精简小图。此举将 Rust 托盘守护进程的内存开销从数十兆断崖式压缩至 KB 级别，并大幅提升显示锐度。
* **FUSE 沙盒穿透与稳健渲染**: 针对 AppImage 环境下挂载点权限隔离导致的图标显示失败问题，开发了“安全区缓存释放”机制。彻底绕过底层拦截与桌面环境的缓存刷新依赖，实现任意 Linux 发行版 100% 图标渲染成功率。
* **内核级抽象套接字升级 (Abstract Sockets)**:
  * 彻底重构了 Python 主引擎与 Rust 托盘之间的底层 IPC 通信管道。从传统的物理文件级 UDS（基于 `/tmp` 目录）全面跃升为 Linux 独有的**抽象命名空间套接字 (Abstract UDS)**。
  * **零残留与绝对自愈**: 彻底消灭了物理 Socket 文件的生成。现在，通信通道仅存在于内核内存中，并与进程生命周期“同生共死”。即使程序遭遇极端崩溃或被强杀 (`kill -9`)，Linux 内核也会在瞬间自动回收通道，彻底根除了过往因文件残留导致的“地址已被占用”启动报错，将程序的抗灾自愈能力推向 100%。
  * **代码极致瘦身**: 摒弃了过往所有用于权限设定与垃圾文件清理的冗余防御性代码，实现了底层架构的极简暴力美学。

### 稳定性提升 (Stability & Security)
* **智能轮询与重试机制**: 优化冷启动场景，通过自动重试有效解决了因 Rust 托盘初始化时滞导致的首发状态推送丢失问题。
* **安全沙盒强化**: 为所有 UDS 通信管道强制实施 `0o600` 权限控制，确保底层 IPC 通信仅限当前用户，防御潜在提权风险。
* **进程生命周期优化**: 修复了重复激活应用时的重入 Bug，严格校验初始化状态，防止托盘与 IPC 服务的冗余创建。

---

## v0.11.2 (2026-02-22)
### BUG修复 
- 系统托盘重构：彻底修复了 AppImage 打包版因底层沙盒限制，导致系统托盘（Tray）图标无法正常显示、以及右键菜单点击无反应的问题。
- 轮换逻辑修正：修复了壁纸自动轮换机制中的“幽灵计时器”漏洞。解决了在手动点击“停止壁纸”后，后台定时器依然运行并在倒计时结束后强制触发壁纸切换（诈尸）的问题。现在的轮播状态已与壁纸生命周期完美同步。

## v0.11.1 (2026-02-21)
### BUG修复
- 修复了在某些 Linux 发行版上 AppImage 软件包无法正确识别系统图标主题的问题。现在 AppImage 包含了必要的图标资源，并且在运行时会优先使用系统图标，确保界面图标显示正常。
- 修复了 AppImage 无法正常重启的问题。现在 AppImage 包含了完整的重启逻辑，用户在使用过程中点击重启按钮时，应用会正确关闭当前实例并启动新的实例，确保重启功能的正常使用。


## v0.11.0 (2026-02-19)
### 发布AppImage软件包（*\.appimage）与Arch软件包（\*.pkg.tar.zst ）
- 发布了适用于Linux的AppImage软件包，用户可以直接下载运行，无需安装，提供了极大的便利性和兼容性
- 同时发布了可以使用`sudo pacman -U`本地安装的Arch软件包，方便Arch用户通过包管理器进行安装。
  - 暂未发布到AUR仓库，正在准备中...
- **特别注意**：AppImage和Arch软件包目前仅包含GUI应用程序，不包含后端引擎。用户需要先安装后端引擎，然后才能使用GUI进行壁纸管理。
  - 后续计划在软件包中集成后端引擎，提供一键安装体验，目前正在评估技术方案和兼容性问题。
- **特别注意**：发布初期可能会遇到一些兼容性问题或用户反馈，我们将持续跟进并快速发布修复版本，确保用户体验的稳定和流畅。
  - 我们非常欢迎用户在GitHub Issues中反馈使用过程中遇到的任何问题或建议，以帮助我们不断改进和优化应用程序。
- **特别注意**：虽然我们已经尽力测试了AppImage和Arch软件包的兼容性，但由于Linux发行版的多样性，仍可能存在一些特定环境下的问题。如果您在使用过程中遇到任何问题，请务必在GitHub Issues中提供详细的环境信息和错误日志，以便我们能够快速定位和解决问题。
  - 我们也计划在未来的版本中增加对其他Linux发行版的软件包支持，如Debian/Ubuntu的DEB包、Fedora的RPM包等，以满足更多用户的需求。
  - 总之，我们非常期待这个新版本的发布，并希望它能够为Linux用户带来更好的壁纸管理体验。感谢大家的支持和反馈，我们会继续努力改进和优化应用程序！

## v0.10.6 (2026-02-15)
### UI修复（UI fixes）
- 修复了`Performance`与`Settings`下的各项没有边框包裹的视觉问题
  - 现在各项都会有一个淡灰色的边框包围，并且微调了各项之间的行距，更加协调
- 修复了`Settings`下开关按钮（toggle switches）的视觉问题：
  - 为浅色模式与深色模式下都适配了合适的开关滑块颜色与滑动轨道（track）的颜色
- 修复了`Home`下`Grid`与`List`相互切换时的强调色显示问题

### 性能修复
- 修复了在切换Grid与List的时候可能会出现严重的滞后，响应慢的问题
...
### Bug 修复
- **图标显示**：修复了应用在 Dock/任务栏中显示默认图标的问题。通过将 `pic/icons/` 目录注册到图标主题、设置程序运行名（prgname）以及优化 `.desktop` 文件生成逻辑（增加 `StartupWMClass` 并同步 ID 命名），实现了窗口与自定义图标的完美关联
