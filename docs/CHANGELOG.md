...
### Core Breakthrough: Bi-directional IPC & Multi-dimensional Status
- Industrial-grade Bi-directional IPC:
  - Established a full-duplex communication link based on Unix Domain Sockets (UDS). This grants the minimalist Rust tray the ability to "talk" with the Python main process in real-time.
  - Implemented a status synchronization heartbeat and debouncing (diffing) logic, ensuring tray tooltips stay in millisecond-sync with the GUI while maintaining zero socket load when idle.
