...
## Tauri + React + Rust era

## v2.0.0 (2026-03-24)

### Architecture Migration

Complete rewrite from Python/GTK to Tauri/React/Rust:

- **Frontend**: Python 3.10+ / GTK4 / Libadwaita → React 19 / TypeScript / Tailwind CSS / shadcn/ui
...
- **Playlist System**: Create, edit, and cycle through custom wallpaper collections with a dedicated sidebar
...
- **Enhanced Performance Monitor**: Real-time CPU/Memory charts with per-process breakdown (Frontend, Backend, Engine)
- **Tauri v2**: Leverages the latest Tauri features for better OS integration and security
- **XDG Compliance**: Config, state, and cache files now follow XDG Base Directory specifications

### Configuration Changes

Configuration split from a single `config.json` into six specialized files:
...
Settings tabs changed from General/Audio/Advanced to:
...
- Only `--hidden` parameter is supported for background startup
- Removed legacy CLI commands (`--show`, `--hide`, `--toggle`, `--random`, `--stop`, `--quit`)

### Removed Features

- System accent color sync (theme now uses system dark/light preference only)
- Web wallpaper property injection panel (Wayland limitations)
- Desktop shortcut auto-creation (use AppImage or package manager instead)

### Dependencies

- Requires `webkit2gtk-4.1` (no longer needs `libadwaita` or `gtk4`)

---

## Legacy Python/GTK era (archived)

<details>
<summary>Expand legacy changelog</summary>

> [!NOTE]
> 🌐 Language Note: This English documentation was generated by AI and translating tools. While we strive for accuracy, some technical nuances might be lost. If you spot any linguistic errors, please feel free to open an issue or submit a Pull Request. Your help is much appreciated!
...
- **Compact Preview Mode**:
  ...
  - **Full Functionality**: Includes core features such as Apply Wallpaper, Stop Wallpaper, Lucky (random), and Jump to Current Wallpaper.
